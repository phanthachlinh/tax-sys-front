...
import { ClientTypes } from '../ducks/client/client.enum';
...
class ClientsPage extends React.Component<ClientsPageProps, ClientsPageState>{
	...
	addClientHandler(e: any) {
		...
		if (fail) return
			...
			this.props.postClient(object, this.state.page);
		}
	...
	showUpdateFormHandler(id: string) {
		this.setState({ showUpdateForm: true, updatingUserID: id })
	}
	closeAddFormHandler() {
	...
	changePageHandler(page: number, searchTerm: string) {
		this.props.getClients(page, searchTerm)
	}
	...
	render() {
		let clientsJSX = this.props.clients.results.map((client: IClient) => {
			...
			return (
				<ClientListItem key={client._id} position={position} client={client} history={this.props.history} showUpdateForm={this.showUpdateFormHandler.bind(this, client._id)} />
			)
		})
		return (
			<div>
				...
				<Pagination getClientsHandler={this.changePageHandler.bind(this)} searchTerm={this.state.searchTerm} activePage={this.props.clients.page} totalCount={this.props.clients.count} />
			</div>
		)
	}
}
