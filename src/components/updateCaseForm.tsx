...
class UpdateCaseForm extends React.Component<IProps, { case: ICase, newCountry: string, newStatus: number }>{
	...
	componentDidUpdate(prevProps: any) {
		...
		if (prevProps.updatingCase._id !== this.props.updatingCase._id) {
			this.setState({ case: this.props.updatingCase, newCountry: this.props.updatingCase.country, newStatus: this.props.updatingCase.status })
		}
	}
	updateCaseHandler(ev: any) {
		ev.preventDefault()
		this.props.updateCase(this.props.updatingCase._id, this.state.newStatus, this.state.newCountry)
	}
	changeCountryValueHandler(ev: any) {
		this.setState({ newCountry: ev.target.value })
	}
	changeStatusValueHandler(ev: any) {
	...
	render() {
		...
		*/
		...
		let statusOptionJSX = []
		for (let i = 0; i < 8; i++) {
			statusOptionJSX.push(<option key={i} value={i}>{i}</option>)
		}
		...
		return (
			<UpdateFormWrapper show={this.props.show}>
				<StyledForm id="updateForm">
					<select value={this.state.newStatus} onChange={this.changeStatusValueHandler.bind(this)}>
						{statusOptionJSX}
					</select>
